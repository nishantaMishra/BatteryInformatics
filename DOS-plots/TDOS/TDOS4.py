"""Working as intended on Thu May  1 08:02:52 PM EDT 2025

Program plots total density of states by reading TDOS.dat file obtained from vaspkit.
This version includes bandgap detection and analysis of magnetic properties.
Bandgap detection is based on the presence of zero density of states in both spin channels.
"""
...
def detect_bandgap_exact(energy, tdos_up, tdos_down):
    ...
    # Fermi level index
    # Fermi level is the energy with the minimum absolute value

    fermi_idx = np.argmin(np.abs(energy))

    # Last zero before Fermi (VBM)
    vbm = None
...
def analyze_tdos(energy, tdos_up, tdos_down,
                 symmetry_tol=0.01):
    print("\n----- Analysis Results -----")
...
def plot_tdos(data, directory):
    energy = data[:, 0]
    up = data[:, 1]
    down = data[:, 2]

    bandgap_info = analyze_tdos(energy, up, down)
    avg = 0.5 * (up + np.abs(down))

    plt.figure(figsize=(8, 6))
    plt.plot(energy, up, label='TDOS-UP', color='C0')
    plt.plot(energy, down, label='TDOS-DOWN', color='C1')
...
if __name__ == "__main__":
    ...
    user_dir = input("Path to directory containing TDOS.dat: ")
    path = os.path.join(user_dir, "TDOS.dat")
    dos_data = read_tdos_file(path)
    if dos_data is not None:
        plot_tdos(dos_data, user_dir)
