# Working as intended on Wed Jun 11 07:47:41 PM EDT 2025
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                if tokens and tokens[0].lower() in ['up', 'down']:
                    spin_filter = tokens[0].upper()
                    plotting_input = plotting_input[len(tokens[0]):].strip()
                elif tokens and tokens[-1].lower() in ['up', 'down']:
                    spin_filter = tokens[-1].upper()
                    plotting_input = plotting_input[:-(len(tokens[-1]))].strip()
