...
def update_plot(z_value, show_above=True, show_below=True, preserve_view=True):
    ...
    # Add instructions for atom selection
    instructions = "Click: Select atom | Backspace: Remove selected | Ctrl+Z: Undo last removal"
...
def on_key(event):
    ...
    if event.key == 'backspace' and selected_atom is not None:
        if selected_atom not in excluded_atoms:
            ...
            print(f"Atom #{selected_atom} ({elems[selected_atom]}) excluded from consideration")
        update_plot(z_slider.val, check.get_status()[0], check.get_status()[1], preserve_view=True)
...
def clear_exclusions(event):
    ...
    excluded_atoms = set()
    exclusion_stack = []
    print("All atom exclusions cleared")
    update_plot(z_slider.val, check.get_status()[0], check.get_status()[1], preserve_view=True)
